...
const DEFAULT_CATEGORIES = ["Groceries", "Parties", "Travel", "Restaurants", "Transport", "Other"];
...
export default function Spending({ session }) {
  ...
  const periodEnd = period === "week" ? t("sp.thisWeekWord") : t("sp.thisMonthWord");
  ...
  async function remove(item) {
    if (!window.confirm(t("sp.deleteConfirm"))) return;
    ...
    <>
      ...
      <main className="page">
        ...
        <div className="period-nav">
          <button className="btn btn-small" aria-label={t("sp.prevPeriod")} onClick={() => goTo(offset - 1)}>
            ‹
          </button>
          <strong>{range.label}</strong>
          <button
            className="btn btn-small"
            aria-label={t("sp.nextPeriod")}
        ...
        {offset !== 0 && (
          <button className="link small today-link" onClick={() => goTo(0)}>
            {period === "week" ? t("sp.backToWeek") : t("sp.backToMonth")}
          </button>
        )}
        ...
        <section className={cardClass} aria-live="polite">
          {limitCents > 0 ? (
            <>
              ...
              <ProgressBar value={spentCents} max={limitCents} label={t("sp.budgetUsed")} />
              <p className="balance-sub">
                {t("sp.spentOf", { spent: eur(spentCents / 100), total: eur(limitCents / 100) })}
                {offset === 0 && remainingCents > 0 &&
                  t("sp.perDay", { amount: eur(remainingCents / 100 / daysLeft(range.to)), period: periodEnd })}
              </p>
            </>
          ) : (
            <>
        ...
        <section className="block">
          <h2>{t("sp.calendar")}</h2>
          <p className="muted small hint">{t("sp.tapDay")}</p>
          <CalendarGrid
            from={range.from}
            to={range.to}
            totals={totals}
