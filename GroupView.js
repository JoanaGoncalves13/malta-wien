...
export default function GroupView({ groupId, session }) {
  ...
  const nameOf = (id) => members.find((m) => m.id === id)?.name ?? "Alguém";
  ...
  async function settle(debt) {
    const text = `Confirmar que ${nameOf(debt.from)} já pagou tudo o que devia a ${nameOf(debt.to)} (${eur(debt.cents / 100)})?`;
  ...
  async function removePayment(payment) {
    if (!window.confirm("Apagar este pagamento? Os saldos vão ser recalculados.")) return;
